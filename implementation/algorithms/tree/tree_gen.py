...
@typechecked
def tree_gen(move_list : list, predicted_turns : int) -> tree_node.TreeNode:
    #global history
...
@typechecked
def tree_generation_recursive(current_predicted_moves : int , current_node : tree_node.TreeNode, beginning_state : chess.Board, max_predicted_moves : int) -> None :
...
@typechecked
def construct_current_state(board : chess.Board, move_list : list) -> chess.Board:
    i = len(move_list)
    while i > 0:
